...
from mainbot.constants import TUPLE_DIGITS_BASE64


def get_beg_info_element(timestamp: int):
    current_date = datetime.fromtimestamp(timestamp)
    current_month = current_date.month
    if 8 <= current_month <= 12:
        beg_element = current_date.element
    else:
        beg_element = current_date.element - 1
    return beg_element
...
def get_beg_end_date():
    ...
    beg_date = datetime(beg_element, 9, 1)
    end_date = datetime(beg_element + 1, 9, 1)
